...
export function MobileLayout({ children, onClose, fontZoom = 1 }) {
  ...
  // Standalone build: the app IS the page, so there is nothing to close back
  // to. When no `onClose` is handed in, Escape, the backdrop tap and the close
  // button all stand down — the frame is otherwise unchanged.
  useEffect(() => {
    if (!onClose) return;
